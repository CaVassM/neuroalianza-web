...
type Paso = 'servicio' | 'sentimiento' | 'indicacion' | 'cierre';
...
export const RegistroCitaModal: React.FC<Props> = ({
  ...
  const confirmar = () => {
    ...
    onConfirmar({ servicio, sentimiento, indicacion });
    cerrar();
  };
  ...
  const indice = ['servicio', 'sentimiento', 'indicacion'].indexOf(paso);
          ...
          >
                ...
                <>
                  ...
                  {flujo.indicaciones.map((ind) => (
                    <button
                      ...
                      className="w-full text-left p-4 rounded-xl border border-[#E5E1EC] hover:border-[#4A2270] hover:bg-[#FAF8FD] transition-all text-[14px] font-medium text-[#2E2A33] cursor-pointer"
                    >
                      {ind.etiqueta}
                    </button>
                  ))}
            ...
            {paso !== 'servicio' && paso !== 'cierre' && (
              <div className="px-6 pb-5">
                <button
                  type="button"
                  onClick={() => setPaso(paso === 'indicacion' ? 'sentimiento' : 'servicio')}
